...
const DATASET_KEY = 'grade3-basic-english-premium';
const FILE = path.join(__dirname, 'grade3-basic-english-premium.json');
...
(entries || []).forEach((entry, index) => {
  ...
  if (!/^g3-\d{4}$/.test(entry.id || '')) addError(id, 'ID must match g3-0001 format.');
...
console.log('Grade 3 vocabulary validation summary');
console.log('-------------------------------------');
